---
name: build-report
description: Generate a daily, weekly, or monthly Aidan performance report — calls, chats, conversions, agent performance, billing usage. Use when the user asks for a "report", "summary", "how did we do", "weekly recap", "month in review", or any variation of "tell me what happened" over a time window.
---

# Build Report

Produce a tight executive-style report for the active Aidan client over a
...
1. **Confirm context.** `mcp__aidan__show_client` — abort with a prompt if
   no active client.
