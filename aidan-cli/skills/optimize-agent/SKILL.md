...
1. **Confirm context.** Call `mcp__aidan__show_client`. If no active
   client, ask the user which client and run `mcp__aidan__use_client`.

2. **Locate the agent.**
   - If the user gave an id, use it.
   - Otherwise call `mcp__aidan__list_records resource="agents"` and
     either pick the obvious match or ask the user.

3. **Pull the current config.**
   `mcp__aidan__get_record resource="agents" record_id=<id>` to see the
   current system prompt, tools, and any guardrails.

4. **Pull recent samples.** Use `mcp__aidan__run_operation`:
...
7. **Confirm before applying.** Show the proposed update; ask the user to
   ...
   `mcp__aidan__update_record resource="agents" record_id=<id>` with the
   new prompt.

8. **Suggest a verification plan.** "Re-run this analysis in a week" or
   "place a test call with `mcp__aidan__send_ai_call`" — whichever fits.
