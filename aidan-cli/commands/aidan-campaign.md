...
> /aidan-campaign add 500 contacts with a phone number that haven't been
...
1. Call `mcp__aidan__show_client`.
2. If the user mentioned a client by name in `$ARGUMENTS` ("...in the
   ...
   - On yes, call `mcp__aidan__use_client identifier="Husband Realty"`.
3. If still no active client, abort and ask `/aidan-use-client <name>` first.
...
1. Call `mcp__aidan__jarvis_query tool_name="list_workflows" args={}`.
...
Translate the user's natural language into the Aidan contact filter spec.
...
```
mcp__aidan__run_operation operation="contacts.list"
...
3. For each batch:
   ```
   mcp__aidan__add_contacts_to_workflow workflow_id="<wf-id>"
   contact_ids=[...]
   ```
...
- For destructive-feeling phrases like "send SMS to all contacts now",
  refuse — point them at `/aidan-campaign` for proper enrollment.
