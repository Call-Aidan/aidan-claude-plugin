...
1. Call `mcp__aidan__show_client`. If `effective_company_id` is empty, abort.
2. If `$ARGUMENTS` is empty, ask the user for the agent id (or call
   `mcp__aidan__list_records resource="agents"` and let them pick).

### Pull

1. Call `mcp__aidan__get_record` with `resource="agents"` and
   `record_id="$ARGUMENTS"`.
2. Call `mcp__aidan__run_operation` with `operation="agent_tools.list"`
   and `path_params={agent_id: "$ARGUMENTS"}` to fetch the agent's tool list.
3. Call `mcp__aidan__run_operation` with `operation="agent_files.list"`
...
- `README.md` — short note explaining how to push edits back via
  `/aidan-push-agent` or `mcp__aidan__update_record`
