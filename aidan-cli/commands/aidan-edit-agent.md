...
2. If `$ARGUMENTS` is empty, run `mcp__aidan__jarvis_query
   tool_name="list_agents" args={}` and let the user pick.
3. Pull the current state:
   `mcp__aidan__get_record resource="agents" record_id="$ARGUMENTS"`.
...
```
mcp__aidan__update_record resource="agents" record_id="$ARGUMENTS"
data={...changed fields only...}
```

On success, briefly confirm what changed (one line) and offer:

> Want me to test it with a call (`mcp__aidan__send_ai_call`) or run
