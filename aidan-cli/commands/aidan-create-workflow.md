...
2. Call `mcp__aidan__describe_operation` with `operation="workflows.create"`
...
verify supported action types, call `mcp__aidan__run_operation` with
`operation="workflows.list"` and inspect a recent example.

### Create

Call `mcp__aidan__create_record` with `resource="workflows"` and the
assembled payload. On success, show the workflow id and offer to add the
first contacts via `mcp__aidan__add_contacts_to_workflow`.
