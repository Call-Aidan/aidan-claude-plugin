---
description: Push local agent edits back to Aidan
argument-hint: [agent-slug-or-path]
---

Push edits from a previously cloned agent back to Aidan.

### Find the agent

1. If `$ARGUMENTS` is provided, look for `agents/$ARGUMENTS/aidan-agent.json`.
...
1. Call `mcp__aidan__show_client`. If `effective_company_id` is empty or
...
2. Call `mcp__aidan__get_record` with `resource="agents"` and the id, and
...
4. Call `mcp__aidan__update_record` with `resource="agents"`, the id, and
