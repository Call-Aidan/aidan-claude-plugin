...
//   ~/.aidan/credentials.json (written by /aidan-cli:aidan-sign-in),
//   then execs `npx mcp-remote` which acts as the actual stdio<->HTTP
//   bridge to the hosted Aidan MCP at api.callaidan.com/mcp.
//
...
const REMOTE_URL = 'https://api.callaidan.com/mcp';
const CREDENTIALS_PATH = path.join(os.homedir(), '.aidan', 'credentials.json');
...
function readToken() {
  ...
  return (process.env.AIDAN_TOKEN || '').trim() || null;
}
...
if (!token) {
  ...
  process.stderr.write(
    '[aidan-cli] No Aidan token found. Run /aidan-cli:aidan-sign-in to authenticate.\n'
  );
  process.exit(1);
}
...
child.on('error', (err) => {
  process.stderr.write(`[aidan-cli] Failed to spawn mcp-remote: ${err.message}\n`);
  process.exit(1);
});
